...
function pembelianController($scope, pembelianServices, helperServices, pesan) {
    ...
    $scope.edit = (param) => {
        ...
        $scope.produk = $scope.datas.produks.find(x => x.id_produk == param.id_produk);
        console.log($scope.produk);
        $scope.variant = $scope.produk.variant.find(x => x.id_variant == param.id_variant);
        // console.log(param);
        $("#modals-default").modal("show");
    }
    $scope.save = (param) => {
        if (!param.id_pembelian) {
            ...
            pembelianServices.put(param).then((res) => {
                var data = $scope.datas.pembelian.find(x => x.id_pembelian == param.id_pembelian);
                if (data) {
...
function penjualanController($scope, penjualanServices, helperServices, pesan) {
    ...
    $scope.check = (cari, ukuran) => {
        if(cari.length > 1) {
            $scope.dataFilter = $scope.datas.filter(x => x.nama_produk.toLowerCase().includes(cari.toLowerCase()));
            if(ukuran) {
                if($scope.dataFilter.length > 0) {
                    $scope.dataFilter = $scope.dataFilter.filter(x => x.ukuran == ukuran);
                }
            }
        }else{
            $scope.dataFilter = [];
    ...
    $scope.keranjang = (param) => {
        var data = $scope.dataKeranjang.find(x => x.id_variant == param.id_variant);
        if( data) {
            data.qty += 1;
    ...
    $scope.save = (param) => {
        if (!param.id_pembelian) {
            param.tanggal_pembelian = helperServices.dateToString(param.tanggal_beli);
            penjualanServices.post(param).then((res) => {
                $scope.datas.pembelian.push(res);
                $("#modals-default").modal("hide");
                $scope.model = {};
                pesan.Success("Data berhasil disimpan", "Success", "info");
            ...
            penjualanServices.put(param).then((res) => {
                var data = $scope.datas.pembelian.find(x => x.id_pembelian == param.id_pembelian);
                if (data) {
                    data.tanggal_pembelian = param.tanggal_pembelian;
                    data.qty = param.qty;
